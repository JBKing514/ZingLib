...
_BOOTSTRAP_CACHE: dict[str, Any] = {"ts": 0.0, "dsn": "", "status": None}
_BOOTSTRAP_TTL_S = 3.0
...
def _invalidate_bootstrap_cache() -> None:
    with _BOOTSTRAP_CACHE_LOCK:
        ...
        _BOOTSTRAP_CACHE["dsn"] = ""
        _BOOTSTRAP_CACHE["status"] = None
...
def ensure_auth_schema(dsn: str) -> None:
    """Make sure the account/session tables exist.

    The account tables are owned by the shared migration baseline. The function
    keeps its name and its lazily-triggered call
    sites -- a DSN stored at runtime by the setup wizard still self-heals -- but
    it no longer carries a copy of the schema.
    """
    dsn_s = str(dsn or "").strip()
    if not dsn_s:
        raise ValueError("missing dsn")
    ok, msg = ensure_schema(dsn_s)
    if not ok:
        raise RuntimeError(f"auth schema not ready: {msg}")
...
def bootstrap_status(dsn: str) -> dict[str, Any]:
    ...
    now = time.monotonic()
    with _BOOTSTRAP_CACHE_LOCK:
        cached = _BOOTSTRAP_CACHE.get("status")
        if (
            cached is not None
            and str(_BOOTSTRAP_CACHE.get("dsn") or "") == dsn_s
            and (now - float(_BOOTSTRAP_CACHE.get("ts") or 0.0) < _BOOTSTRAP_TTL_S)
        ):
            return dict(cached)

    ensure_auth_schema(dsn_s)
    with psycopg.connect(dsn_s, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
    ...
    with _BOOTSTRAP_CACHE_LOCK:
        ...
        _BOOTSTRAP_CACHE["dsn"] = dsn_s
        _BOOTSTRAP_CACHE["status"] = dict(out)
    return out
...
def get_session_user(dsn: str, token: str, ttl_hours: int = 24) -> dict[str, Any] | None:
    ...
    with psycopg.connect(dsn, row_factory=dict_row) as conn:
        with conn.cursor() as cur:
            ...
            half_ttl_sec = (max(1, ttl_hours) * 3600) / 2
            if remaining_sec < half_ttl_sec:
                new_expire = now.timestamp() + max(1, int(ttl_hours)) * 3600
                exp = datetime.fromtimestamp(new_expire, tz=timezone.utc)
                cur.execute(
                    "UPDATE ui_sessions SET expires_at=%s, last_seen_at=%s WHERE sid=%s::uuid",
                    (exp, now, str(row.get("sid") or "")),
                )
                extended = True
            else:
