...
def _download_siglip_worker(task_id: str, model_id: str) -> None:
    try:
        ...
        _set_dl_state(task_id, {"progress": 45, "stage": "download_model_files"})
        py_dl = (
            "from huggingface_hub import snapshot_download; "
            f"snapshot_download(repo_id='{model_id}', cache_dir=r'{siglip_dir.as_posix()}', "
            "ignore_patterns=['*.msgpack', '*.h5', '*.ot']); "
            "print('download_ok')"
        )
        rc, out, err = _run_cmd([sys.executable, "-c", py_dl], env_extra=env_extra, timeout=7200)
        _append_dl_log(task_id, out[-1200:] if out else "")
        _append_dl_log(task_id, err[-1200:] if err else "")
        if rc != 0:
            raise RuntimeError(f"model download failed: {err or out}")

        _set_dl_state(task_id, {"progress": 90, "stage": "verify_model"})
        py_v = (
            "from transformers import AutoProcessor, AutoTokenizer; "
            f"AutoProcessor.from_pretrained('{model_id}', cache_dir=r'{siglip_dir.as_posix()}', local_files_only=True); "
            f"AutoTokenizer.from_pretrained('{model_id}', cache_dir=r'{siglip_dir.as_posix()}', local_files_only=True); "
            "print('verify_ok')"
        )
        rc_v, out_v, err_v = _run_cmd([sys.executable, "-c", py_v], env_extra=env_extra, timeout=600)
        _append_dl_log(task_id, out_v[-1200:] if out_v else "")
        _append_dl_log(task_id, err_v[-1200:] if err_v else "")
        if rc_v != 0:
            raise RuntimeError(f"model verification failed: {err_v or out_v}")
