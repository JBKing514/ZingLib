...
def run_task(task_name: str, cmd: list[str], timeout_s: int = 1800) -> dict[str, Any]:
    ...
    def _task_error_hint(task: str, stderr_txt: str) -> str:
        ...
        if task == "local_ingest":
            ...
            return "Hint: check Local ingest logs for VL/EMB service connectivity or model loading errors."
        return ""
...
def resolve_task_command(task_name: str, args_line: str = "") -> list[str]:
    ...
    if normalized_task == "local_ingest":
        cmd = f"{py} -u /app/vectorIngest/worker_vl_ingest.py --dsn \"$POSTGRES_DSN\""
        if args_q:
            cmd = f"{cmd} {args_q}"
        return ["bash", "-lc", cmd]

    if normalized_task in TASK_COMMANDS:
        return TASK_COMMANDS[normalized_task] + args
