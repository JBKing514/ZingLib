...
Schema:
  See textIngest/migrations/0001_baseline.sql (works + read_events).
...
def main(argv: list[str]) -> int:
    ...
    ap.add_argument(
        "--schema",
        default=str(Path(__file__).resolve().parent / "migrations" / "0001_baseline.sql"),
        help=(
            "Path to the baseline schema (used with --init-schema). Only the "
            "baseline is applied here; use run_migrations.sh for the full set."
        ),
    )
    ap.add_argument(
        "--init-schema",
