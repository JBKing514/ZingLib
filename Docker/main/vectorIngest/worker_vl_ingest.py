#!/usr/bin/env python3
"""Local library -> VLM description + embeddings -> Postgres (pgvector)

Pipeline per arcid:
  - Read cover + 3 random pages from runtime/local_lib/<local_dir>
...
class OpenAICompatClient:
    ...
    def chat_completions(self, model: str, messages: list[dict[str, Any]], temperature: float = 0.2, max_tokens: int = 800) -> str:
        ...
        if (content is None or (isinstance(content, str) and not content.strip())) and isinstance(reasoning_content, str) and reasoning_content.strip():
...
def _safe_local_gallery_root(local_root: Path, local_dir: str) -> Path:
    root = local_root.resolve()
    rel = str(local_dir or "").replace("\\", "/").strip().strip("/")
    if not rel:
        raise RuntimeError("local_dir missing")
    gallery = (root / rel).resolve()
    if os.path.commonpath([str(root), str(gallery)]) != str(root):
        raise RuntimeError(f"unsafe local_dir: {local_dir}")
    if not gallery.exists() or not gallery.is_dir():
        raise RuntimeError(f"local gallery folder missing: {gallery}")
    return gallery


def _list_local_pages(gallery_root: Path) -> list[Path]:
    exts = {".jpg", ".jpeg", ".png", ".webp", ".avif", ".gif"}
    pages: list[Path] = []
    for p in gallery_root.rglob("*"):
        if not p.is_file():
            continue
        if str(p.suffix or "").strip().lower() not in exts:
            continue
        pages.append(p)
    pages.sort(key=lambda x: str(x.relative_to(gallery_root)).lower())
    return pages


def _choose_cover_local_page(page_paths: list[Path]) -> Path | None:
    if not page_paths:
        return None
    candidates: list[tuple[int, str, Path]] = []
    for p in page_paths:
        fn = p.name
        if _is_thumb_filename(fn):
            continue
        idx = _page_index_from_filename(fn)
        if idx is None:
            continue
        candidates.append((idx, fn, p))
    if candidates:
...
def _pick_images_local(local_root: Path, local_dir: str, rng: random.Random, k_random_pages: int = 3) -> list[bytes]:
    gallery = _safe_local_gallery_root(local_root, local_dir)
    pages = _list_local_pages(gallery)
    if not pages:
        raise RuntimeError(f"No usable local pages found: {gallery}")

    cover = _choose_cover_local_page(pages)
    if cover is None:
        raise RuntimeError(f"No cover page found: {gallery}")

    blobs: list[bytes] = [cover.read_bytes()]
    candidates = [p for p in pages if p != cover and not _is_thumb_filename(p.name)]
    if candidates:
        picks = candidates if len(candidates) <= k_random_pages else rng.sample(candidates, k=k_random_pages)
        for p in picks:
...
def main(argv: list[str]) -> int:
    ...
    ap = argparse.ArgumentParser(description="Generate local library vectors with optional VLM text pipeline")
    ap.add_argument("--dsn", required=True, help="PostgreSQL DSN")
    ap.add_argument("--local-lib-root", default=os.getenv("DATA_UI_LOCAL_LIB_DIR", "/app/runtime/local_lib"), help="Local library root path")
    ...
    ap.add_argument(
        "--vl-media-dir",
        default=os.getenv("VL_MEDIA_DIR", os.path.expanduser("~/.cache/local_worker/media_vl")),
    ...
    local_root = Path(str(args.local_lib_root or "/app/runtime/local_lib")).resolve()
    ...
    select_sql = (
        "SELECT arcid, COALESCE(local_dir, '') AS local_dir FROM works "
        "WHERE COALESCE(source, 'lrr') = 'local' AND COALESCE(local_dir, '') <> '' "
        + (
            "AND (description IS NULL OR description = '' OR desc_embedding IS NULL) "
            if args.only_missing
            else ""
        )
        + "ORDER BY local_dir, arcid"
    )
    ...
    with _connect_db_with_retry(
        ...
        if args.arcid:
            wanted = [str(a or "").strip() for a in args.arcid if str(a or "").strip()]
            if wanted:
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT arcid, COALESCE(local_dir, '') AS local_dir FROM works "
                        "WHERE COALESCE(source, 'lrr') = 'local' AND COALESCE(local_dir, '') <> '' "
                        "AND arcid = ANY(%s::text[]) ORDER BY arcid",
                        (wanted,),
                    )
                    targets = [(str(r[0] or ""), str(r[1] or "")) for r in cur.fetchall()]
                conn.commit()
        if not targets:
            with conn.cursor() as cur:
        ...
        print(f"Will process local works={len(targets)}")

        for i, item in enumerate(targets, start=1):
            arcid, local_dir = item
            t0 = time.time()
            try:
                blobs = _pick_images_local(local_root, local_dir, rng=rng, k_random_pages=3)
                ...
                dt = time.time() - t0
                print(f"[{i}/{len(targets)}] OK {arcid} ({dt:.2f}s) mode=text-only desc_len={len(description)}")

            except Exception as e:
                dt = time.time() - t0
                print(f"[{i}/{len(targets)}] ERROR {arcid} ({dt:.2f}s): {e}", file=sys.stderr)
