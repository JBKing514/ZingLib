...
# The baseline is available for explicit ingest initialization; normal startup
# applies the complete numbered migration set through run_migrations.sh.
TEXT_INGEST_SCHEMA=${TEXT_INGEST_SCHEMA:-${DATABASE_SCHEMA:-${SCRIPT_DIR}/migrations/0001_baseline.sql}}
...
Environment variables:
  ...
  TEXT_INGEST_SCHEMA                      default: textIngest/migrations/0001_baseline.sql
  TEXT_INGEST_INIT_SCHEMA                 default: 0 (delegates to run_migrations.sh)
...
# One DDL path. When the caller asks for schema init, run the full numbered
# migration set -- not just the baseline -- so an upgrade cannot leave the
# database with half the schema applied.
